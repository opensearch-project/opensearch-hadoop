...
class RowSerializationEventConverterTest {
  ...
  def generateEvent(): Unit = {
    ...
    // Spark 4.0 changed StructType.toString() format (no spaces between StructFields)
    assertTrue(rawEvent.contains("StructField(field1,StringType,true)") &&
      rawEvent.contains("StructField(field2,StringType,true)") &&
      rawEvent.contains("StructField(field3,StringType,true)") &&
      rawEvent.contains("[value1,value2,value3]"))
